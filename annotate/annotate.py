...
def getAll(input, alen, evalue, bitscore, identity, queryCol, subjectCol, evalueCol, bitscoreCol, alenCol, pidentCol, db, out, unknown, sep):
    check = True
    query = None
    with open(input, "r") as f:
        ...
        for line in f:
            ...
            if check:
                ...
                if len(ls) < 6:
                    print("Error: invalid number of columns!\n"+input+" has "+str(len(ls))+" columns and at least 6 columns are required!")
                    break
                elif len(ls) <= lastIndexOfInterest:
                    print("Error: invalid number of columns!\n"+input+" has "+str(len(ls))+" columns and the last index of interest is "+str(lastIndexOfInterest)+"!")
                    break
                else:
                    check = False
            query = ls[queryCol]
...
def getBestHits(input, alen, evalue, bitscore, identity, queryCol, subjectCol, evalueCol, bitscoreCol, alenCol, pidentCol, db, out, unknown, sep):
    firstQuery = True
    match = False
    check = True
    query = None
...
createdb_parser.add_argument("--header", help="Indicates the presence of a header in the input file (default: True)", type=str2bool, default=True)
createdb_parser.add_argument("-d","--directory", help="Directory of databases (default: $HOME/.annotate/levelDB)", default="home")

idmapping_parser = subparsers.add_parser("idmapping", description="Translate identifiers from the input using the mapping database")
...
idmapping_parser.add_argument("-b", "--bitscore", help="Minimum bit score of a hit to be considered good (default: 50.0)", type=float, default=50.0)
idmapping_parser.add_argument("-e", "--evalue", help="Maximum e-value of a hit to be considered good (default: 0.00001)", type=float, default=0.00001)
idmapping_parser.add_argument("-l", "--alen", help="Minimum alignment length of a hit to be considered good (default: 50)", type=int, default=50)
idmapping_parser.add_argument("-i", "--identity", help="Minimum percent identity of a hit to be considered good (default: 80)", type=float, default=80)
idmapping_parser.add_argument("-d","--directory", help="Directory of databases (default: $HOME/.annotate/levelDB)", default="home")
idmapping_parser.add_argument("--queryCol", help="Column index (0-based) in which the query ID can be found (default: 0)", type=int, default=0)
idmapping_parser.add_argument("--subjectCol", help="Column index (0-based) in which the subject ID can be found (default: 1)", type=int, default=1)
idmapping_parser.add_argument("--evalueCol", help="Column index (0-based) in which the e-value can be found (default: 10)", type=int, default=10)
idmapping_parser.add_argument("--bitscoreCol", help="Column index (0-based) in which the bit score can be found (default: 11)", type=int, default=11)
idmapping_parser.add_argument("--alenCol", help="Column index (0-based) in which the alignment length can be found (default: 3)", type=int, default=3)
idmapping_parser.add_argument("--pidentCol", help="Column index (0-based) in which the percent identity can be found (default: 2)", type=int, default=2)
...
if args.subparser_name == "createdb":
    ...
    if args.all:
        getAll(args.input, args.alen, args.evalue, args.bitscore, args.identity, args.queryCol, args.subjectCol, args.evalueCol, args.bitscoreCol, args.alenCol, args.pidentCol, db, out, args.unknown, args.sep)
    else:
        getBestHits(args.input, args.alen, args.evalue, args.bitscore, args.identity, args.queryCol, args.subjectCol, args.evalueCol, args.bitscoreCol, args.alenCol, args.pidentCol, db, out, args.unknown, args.sep)
    out.close()
    db.close()
    print("Done!")
    elapsed_time = time.time() - start_time
